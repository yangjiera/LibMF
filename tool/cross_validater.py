...
def cross_validate(dataset, cv=True, filter=""):
    print 'Loading data...\n' + \
    ...
    for i in xrange(5):
        ...
        beta = 0.01
        lamb = 0.00005
        mf = Sparse_BasicMF(alpha, beta, lamb, 100, 1000, O)
...
def get_mask_matrix(X):
    ...
    O = np.zeros((m, n))
    for i in xrange(m):
        # DEPRECATED: this  loop takes time since it iterates over all entries
        # of the data matrix
        '''for j in xrange(n):
            if X[i,j] != 0:
                O[i,j] = 1'''
...
if __name__ == '__main__':
    ...
    #cross_validate('FourCity5', True, filter="amsterdam")
